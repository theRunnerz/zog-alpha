  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        ...
        if (data.alerts) {
            setLogs(data.alerts);
            // Fake a "scanned" counter based on logs just for visuals
            setScanned(data.alerts.length * 1542 + 200); 
        }
    ...
    <div style={{ minHeight: '100vh', background: '#050505', color: '#0f0', fontFamily: 'monospace', padding: '20px' }}>
      ...
      <div style={{ borderBottom: '1px solid #333', paddingBottom: '20px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display:'flex', gap:'10px', alignItems:'center' }}>
             ...
             <Link href="/" style={{ marginLeft:'20px', color: '#0f0', textDecoration:'none', border:'1px solid #0f0', padding:'8px 15px', borderRadius:'5px', fontSize:'14px' }}>EXIT TERMINAL</Link>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '40px' }}>
        ...
        <StatCard label="Defense Actions" value={logs.length} sub="Last 24h" />
      </div>

      {/* LOG FEED */}
      <h2 style={{ fontSize: '16px', borderLeft: '4px solid #0f0', paddingLeft: '10px', marginBottom: '20px', textTransform:'uppercase' }}>Threat Log Stream</h2>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        {logs.length === 0 ? (
          <div style={{ padding: '60px', textAlign: 'center', color: '#333', border: '1px dashed #333', borderRadius:'10px' }}>
            ...
            <div style={{fontSize:'12px', marginTop:'5px'}}>Ensure agent/guardian.js is running in terminal</div>
          </div>
        ) : (
          logs.map((log, i) => (
            <div key={i} style={{ 
              ...
              <div style={{ background: '#111', padding: '15px', borderRadius: '5px', marginBottom: '15px', fontSize: '13px', color: '#ccc', border:'1px solid #333' }}>
                <span style={{ color: '#0f0', fontWeight:'bold' }}>🧠 AI ANALYSIS:</span> "{log.reason}"
              </div>

              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                <div style={{ flex: 1, borderTop: '1px solid #333' }}></div>
                <span style={{ fontSize: '10px', color: '#666', padding:'0 10px' }}>AUTOMATED DEFENSE EXECUTED</span>
                <div style={{ flex: 1, borderTop: '1px solid #333' }}></div>
              </div>

              <div style={{ marginTop: '15px', display:'flex', gap:'20px', fontSize:'12px' }}>
                 <div style={{ color: '#3b82f6', display:'flex', alignItems:'center', gap:'5px' }}>
                    🐦 TWEET POSTED
                 </div>
                 <div style={{ color: 'orange', display:'flex', alignItems:'center', gap:'5px' }}>
                    🚀 RUG-ALERT TOKEN LAUNCHED
                 </div>
              </div>
              
              {/* Fake Tweet Context */}
               <div style={{ marginTop: '10px', color: '#555', fontSize: '10px', fontStyle:'italic' }}>
                 "{log.tweet.substring(0, 80)}..."
               </div>
            </div>
          ))
        )}
